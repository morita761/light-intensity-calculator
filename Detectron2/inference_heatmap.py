...
for i, (mask, box) in enumerate(zip(masks, boxes)):
    ...
    # 中心線に被るか、近すぎる輪郭は無視する
    # バウンディングボックスが中心線をまたぐか、または中心線からの距離が閾値内かをチェック
    ...
    if is_overlapping_or_near:
        # 中心線に被るか近すぎる輪郭は無視
        # デバッグ画像に無視した輪郭を描画したい場合は、ここにロジックを追加
        ...
        all_masks_right.append(mask)
        valid_horseshoe_count_right += 1

print(f"左側の有効な馬蹄形が {valid_horseshoe_count_left} 個、右側の有効な馬蹄形が {valid_horseshoe_count_right} 個検出されました。")
...
# デバッグ画像のプロット
axes[0].imshow(debug_images[0])
axes[0].set_title('Detected Horseshoes')
axes[0].axis('off')

# 左側のヒートマップのプロット
sns.heatmap(heatmap_left, ax=axes[1], cmap='viridis', cbar_kws={'label': 'Normalized Average GFP Intensity'}, vmin=0, vmax=255)
axes[1].set_title('Average GFP Intensity Heatmap (Left)')
axes[1].set_xticks([])
axes[1].set_yticks([])

# 右側のヒートマップのプロット
sns.heatmap(heatmap_right, ax=axes[2], cmap='viridis', cbar_kws={'label': 'Normalized Average GFP Intensity'}, vmin=0, vmax=255)
axes[2].set_title('Average GFP Intensity Heatmap (Right)')
axes[2].set_xticks([])
axes[2].set_yticks([])
