...
for index, file_info in enumerate(image_pairs):
    ...
    image_original_for_rfp = cv2.imread(rfp_file_name)

    if image_original_for_rfp is None:
        print(f"オリジナル画像を読み込めませんでした: {image_original_for_rfp}。スキップします。")
        continue
    ...
    debug_img = image_original_for_rfp.copy()
