...
#      'mask': './pics/mosaic_fz_Fz/20250402_24%APF_Fz-GFP_fz-RNAi_sita_0063/Merged_rfp.tif'},
...
#      'mask': './pics/mosaic_fz_Fz/20250403_again_24%APF_Fz-GFP_fz-RNAi_sita_0063/Merged_rfp.tif'},
# ]
...
for index, file_info in enumerate(image_pairs):
    ...
    for single_horseshoe_mask in all_horseshoe_masks:
        ...
        for i, contour in enumerate(contours):
            ...
            # 【修正1: 分割基点の変更】
            threashold = EXCLUSION_WIDTH # 中心線からの無視する距離を小さくする（データが中心に集中していない場合）
...
if all_aligned_gfp_data_left:
    if(TRIM == 0):
        ...
        data_stack = np.array(all_aligned_gfp_data_left)
        # 1. 軸0（サンプル方向）に沿ってデータをソートする
        sorted_data = np.sort(data_stack, axis=0)
        # 2. 最初と最後の要素（最小値と最大値）を除外する
        trimmed_data = sorted_data[TRIM:-TRIM, :, :]
        # 3. 残ったデータの平均を軸0に沿って計算する
        average_heatmap_left = np.mean(trimmed_data, axis=0)
...
if all_aligned_gfp_data_right:
    if(TRIM == 0):
        ...
        data_stack = np.array(all_aligned_gfp_data_right)
        # 1. 軸0（サンプル方向）に沿ってデータをソートする
        sorted_data = np.sort(data_stack, axis=0)
        # 2. 最初と最後の要素（最小値と最大値）を除外する
        trimmed_data = sorted_data[TRIM:-TRIM, :, :]
        # 3. 残ったデータの平均を軸0に沿って計算する
        average_heatmap_right = np.mean(trimmed_data, axis=0)
...
# =========================================================================
# 【STEP 4】 デバッグ用輝度ヒストグラムの表示
# =========================================================================
# data_left_flat = np.concatenate([arr.flatten() for arr in all_aligned_gfp_data_left])
# data_right_flat = np.concatenate([arr.flatten() for arr in all_aligned_gfp_data_right])
...
plt.hist(data_left_flat, bins=50, range=(0, 255), color='red', alpha=0.6, label='Left Horseshoes')
...
# 右側のヒストグラム
plt.subplot(1, 2, 2)
plt.hist(data_right_flat, bins=50, range=(0, 255), color='blue', alpha=0.6, label='Right Horseshoes')
