...
# 1. 画像の読み込み
image = cv2.imread('1_mask.tif')
if image is None:
    print("画像を読み込めませんでした。パスを確認してください。")
    exit()

green_mask = extract_region(image, coefficient_left=0.28, coefficient_right=0.75, coefficient_bottom=0.8, coefficient_top=0.1)
cv2.imshow('Sample Image', green_mask)
cv2.waitKey(10*1000)

right_image,left_image = split_left_right(green_mask)
# 今はright_imageのみ処理する
image = right_image
