                ...
                if current_section and current_title:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            print(f"警告: ファイル {input_file_path}, 行{line_num}の数値変換に失敗しました: {original_line.strip()} (エラー: {e})")
                            continue
...
def main_normalize_and_save(input_files):
    """
    複数のファイルを読み込み、画像ごとの med_avg でノーマライズし、
    実験ごと・V/DセクションごとにCSVファイルを出力するメイン関数。

    Args:
        input_files: dict形式 {実験名: ファイルパス}
    """

    # 1. データの読み込みと統合
    dfs = []
    for experiment_name, file_path in input_files.items():
        df = _parse_single_file(file_path, experiment_name)
        if not df.empty:
            dfs.append(df)
    ...
    # 4. 実験ごと・V/DセクションごとにCSVファイルを出力

    output_files = {}
    for experiment_name in input_files.keys():
        for section in ['V', 'D']:
            filename = f'{experiment_name}_{section.lower()}.csv'
            df_filtered = df_combined[(df_combined['experiment'] == experiment_name) & (df_combined['section'] == section)]
            output_files[filename] = df_filtered
...
if __name__ == '__main__':
    ...
    INPUT_FILES = {
        'control': 'control.txt',
        'vang_rnai': 'vang_rnai.txt',
        'loco_vang_cas9': 'loco_vang_cas9.txt',
    }

    # 実行
    main_normalize_and_save(INPUT_FILES)
